    ...
    apis: [
      {
        ...
        description: "需要 dataset_manage 权限。",
...
status, data = request_json("POST", "/api/public/datasets",
    ...
    headers=headers)`,
      },
      {
        ...
        description: "需要 dataset_manage 权限。替换数据集关联的 BUC 列表，BUC 必须存在于 wav_buc 表。",
...
for buc in data["items"]:
    print(buc)`,
      },
      {
        ...
        description: "需要 buc_manage 权限。传入 6 个 WAV MD5 与标准点位，系统自动生成一个新的 BUC。",
        body: {
          wave_position_id: {
    ...
    apis: [
      {
        ...
        summary: "获取标注框变动记录",
        description: "可使用 user_id、label_id、buc、start_time、end_time 任意组合筛选；时间按北京时间解释，同时返回页面下拉框需要的人员、标签和 BUC 选项。",
        query: { user_id: "4", label_id: "23", buc: "BUC_000001", start_time: "2026-07-10T00:00", end_time: "2026-07-13T23:59" },
        response: {
...
status, data = request_json("GET", "/api/annotation-changes",
    ...
    query={"user_id": 4, "label_id": 23, "buc": "BUC_000001",
           "start_time": "2026-07-10T00:00", "end_time": "2026-07-13T23:59"},
)
...
for ds in data["items"]:
    print(f"数据集: {ds['name']}  BUC 数: {len(ds['bucs'])}")`,
      },
      {
        ...
        path: "/api/datasets",
        summary: "创建数据集",
        description: "需要 dataset_manage 权限。可同时关联 BUC 列表。",
